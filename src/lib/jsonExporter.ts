...
/**
 * Accepts a raw payload (either an `AtterbergExportPayload` wrapper, a bare
 * `AtterbergProjectState`, or legacy shapes) and returns a normalized
 * `AtterbergProjectState` with a guaranteed `records: AtterbergRecord[]` array.
 * Returns `null` only when the input is not an object at all.
 */
export const extractAtterbergPayload = (value: unknown): AtterbergProjectState | null => {
  if (!isObject(value)) return null;
  const inner = isObject(value.project) ? value.project : value;
  return normalizeAtterbergProjectState(inner);
};
