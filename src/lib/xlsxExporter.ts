...
export const generateAtterbergXLSX = async (
  ...
  for (const record of records) {
    ...
    // Always recompute from trials so exports never show a stale cached value.
    const liquidLimit = calculateLiquidLimit(llTrials);
    const plasticLimit = calculatePlasticLimit(plTrials);
    const linearShrinkage = calculateLinearShrinkage(slTrials);
