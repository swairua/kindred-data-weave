...
const DCPTest = () => {
  ...
  const exportXLSX = async () => {
    await generateTestExcel({ data: { title: "DCP (Dynamic Cone Penetrometer)", ...project, tables: [{ headers: ["Depth (mm)", "Penetration per Blow (mm/blow)"], rows: rows.map(r => [r.depth || "—", r.penetration || "—"]) }] } });
  };

  return (
    <TestSection title="DCP (Dynamic Cone Penetrometer)" onSave={() => {}} onClear={() => setRows([{ depth: "", penetration: "" }])} onExportPDF={exportPDF} onExportXLSX={exportXLSX}>
