...
const GradingTest = ({ testKey }: GradingTestProps) => {
  ...
  return (
    <div className="grading-record space-y-3 pb-6" data-test-key={testKey || "grading"}>
      <div className="flex items-center gap-2 text-xs text-muted-foreground print:hidden"><Button variant="ghost" size="sm" className="h-7 px-2" onClick={() => navigate(-1)}><ArrowLeft className="mr-1 h-3.5 w-3.5" />Back</Button><span>/</span><span>Record test</span></div>
      ...
      <RecordSection title="Soil classification" help="Calculated classification values update from the recorded material data.">
        <div className="grid gap-px overflow-hidden rounded-md border sm:grid-cols-2 lg:grid-cols-3">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 print:hidden"><Button variant="outline" size="sm" className="border-destructive/20 text-destructive hover:bg-destructive/10" onClick={handleClear}><Trash2 className="mr-1.5 h-3.5 w-3.5" />Delete</Button><div className="flex flex-wrap gap-2"><Button variant="outline" size="sm" onClick={() => exportFiles("pdf")}>PDF</Button><Button variant="outline" size="sm" onClick={() => exportFiles("xlsx")}>Excel</Button><Button variant="outline" size="sm" onClick={() => exportFiles("csv")}>CSV</Button><Button size="sm" className="bg-primary" onClick={handleSave} disabled={saveStatus === "saving"}>{saveStatus === "saving" ? <Loader2 className="mr-1.5 h-3.5 w-3.5 animate-spin" /> : saveStatus === "saved" ? <Check className="mr-1.5 h-3.5 w-3.5" /> : <Save className="mr-1.5 h-3.5 w-3.5" />}{saveStatus === "saving" ? "Saving..." : saveStatus === "saved" ? "Saved" : "Save"}</Button></div></div>
    </div>
  );
};
...
const EditableCell = ({ label, value, placeholder, calculated, onChange }: { label: string; value: string; placeholder?: string; calculated?: boolean; onChange?: (value: string) => void }) => calculated ? <CalculatedField label={label} value={value} /> : <RecordField label={label} value={value} placeholder={placeholder} onChange={onChange || (() => {})} />;
