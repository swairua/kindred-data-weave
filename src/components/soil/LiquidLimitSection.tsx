      ...
      <div className="rounded-lg border overflow-hidden">
        <div className="overflow-x-auto">
          <table className="atterberg-table">
            <thead>
              <tr>
                <th className="w-10 sm:w-14">Trial</th>
              ...
              {trials.map((trial, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-1 sm:px-2 py-1.5 hidden md:table-cell atterberg-calculated">
                      <span className="text-xs sm:text-sm">{waterMass !== null ? waterMass : "-"}</span>
                    </td>
                    <td className="px-1 sm:px-2 py-1.5 hidden md:table-cell atterberg-calculated">
                      <span className="text-xs sm:text-sm">{drySoilMass !== null ? drySoilMass : "-"}</span>
                    </td>
                    <td className={cn("px-1 sm:px-2 py-1.5", hasAutoMoisture && "atterberg-calculated")}>
                      {hasAutoMoisture ? (
                        ...
                        <Input
                          ...
                          className={cn("h-7 sm:h-8 text-xs sm:text-sm text-center atterberg-input", started && !valid && !trial.moisture && "border-amber-300")}
                          placeholder="35"
                        />
                      )}
                    </td>
              ...
              <div className="lg:col-span-2 rounded-lg border bg-card p-3">
                ...
                {regressionData && (
                  <p className="mt-2 text-xs text-muted-foreground font-mono">
                    y = {regressionData.slope.toFixed(2)}·log₁₀(x) + {regressionData.intercept.toFixed(2)} &nbsp;|&nbsp; R² = {regressionData.rSquared.toFixed(3)}
                  </p>
                )}
              </div>
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={mergedChartDataExport} margin={{ top: 20, right: 30, left: 60, bottom: 60 }}>
                        ...
                        <XAxis
                          ...
                          ticks={[10, 12, 15, 18, 20, 22, 25, 30, 40].filter((t) => t >= xStartExport && t <= xEndExport)}
                          allowDataOverflow={true}
                          stroke="#000"
                          strokeWidth={2}
