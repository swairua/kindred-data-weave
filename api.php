...
// Refresh session expiry on every request to keep active sessions alive
if (isset($_SESSION['user_id'])) {
    $sessionDb = createSessionDb();
    $sessionId = session_id();
    $refreshSql = "UPDATE `sessions` SET expires_at = DATE_ADD(NOW(), INTERVAL 30 MINUTE) WHERE session_id = ?";
    $refreshStmt = $sessionDb->prepare($refreshSql);
    if ($refreshStmt) {
        $refreshStmt->bind_param('s', $sessionId);
        $refreshStmt->execute();
        $refreshStmt->close();
    }
    $sessionDb->close();
...
{
    $table = trim((string) ($input['table'] ?? $_GET['table'] ?? $_POST['table'] ?? $_REQUEST['table'] ?? ''));
...
try {
    ...
    if ($action === 'list') {
        ...
        // Tables that don't filter by user_id (can be listed by authenticated admins)
        $adminTables = ['users', 'user_allowed_tables'];
        ...
        if ($table === 'test_results') {
            $sql = "SELECT tr.*, p.name as project_name FROM `test_results` tr
                    LEFT JOIN `projects` p ON tr.project_id = p.id
                    $whereClause ORDER BY tr.`$orderBy` $direction LIMIT ? OFFSET ?";
        } else {
